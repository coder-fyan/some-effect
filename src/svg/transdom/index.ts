...
async function parseImageDom(cloneDom: HTMLElement): Promise<HTMLElement> {
    ...
    images.forEach((item, idx) => {
        ...
        let pro: Promise<void> = new Promise((resolve, rejection) => {
            cloneImg.onload = function () {
                let { canvasDom: canvas } = img2canvas(item);
                item.src = canvas.toDataURL(); //the loop onload reason at here: when the img attribute src be init value, it will to get the source and call the onload function, this is why lazy load can be work.
...
export async function parseDom(dom: HTMLElement): Promise<HTMLElement> {
    ...
    //beacuase svg file  can not contain exteral source link in them element, so we need to change the external link to local.
    await parseImageDom(cloneDom);
